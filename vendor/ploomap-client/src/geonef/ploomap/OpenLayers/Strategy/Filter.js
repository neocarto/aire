...
/**
 * ----------- COPIED FROM OL SOURCE BY okapi
 * ----------- as it was missing from Geoportal code
 */
dojo.provide('geonef.ploomap.OpenLayers.Strategy.Filter');
 ...
 *  - <OpenLayers.Strategy>
 */
geonef.ploomap.OpenLayers.Strategy.Filter = OpenLayers.Class(OpenLayers.Strategy, {
    ...
    handleAdd: function(event) {
      //console.log('filter add', this, arguments, this.caching);
        if (!this.caching) {
            ...
            for (var i=0, ii=features.length; i<ii; ++i) {
                ...
                if (this.filter.evaluate(feature)) {
                  ...
                  if (this.cache.filter(
                        function(f) { return f.fid === feature.fid; }).length === 0) {
                        this.cache.push(feature);
                    }/* else {
                      console.log('not adding f:', feature.attributes.name);
                    }*/
                }
            }
        }
      //console.log('end filter add', event, features);
    },
    ...
    handleRemove: function(event) {
      //console.log('filt remove', this, arguments, this.caching);
        if (!this.caching) {
            this.cache = [];
        }
    },
    ...
    CLASS_NAME: "geonef.ploomap.OpenLayers.Strategy.Filter"
